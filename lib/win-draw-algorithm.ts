...
async function checkTeamPerformanceAgainstTopTeams(teamId: number, competitionId: number) {
  // Get matches against top 5 teams
  ...
  for (const m of upcomingMatches) {
    ...
    if (result) {
      ...
      try {
        ...
        const aiAnalysis = await aiService.analyzeMatch(matchData, 'WIN_DRAW')
        aiExplanation = aiAnalysis.explanation
        aiConfidence = aiAnalysis.confidence
        aiKeyFactors = aiAnalysis.keyFactors
      } catch (error) {
        console.warn('[AI] Failed to get AI analysis for Win/Draw:', error)
      }
